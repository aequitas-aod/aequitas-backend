        ...
        if project is not None:
            ...
            for key, value in self.produce_info(dataset_id, dataset):
                ...
                self.logger.error(
                    ...
                    key, updated_project.id, value.replace("\n", "\\n")
                )

    def produce_info(self, dataset_id: str, dataset: pd.DataFrame) -> Iterable[tuple[str, str]]:
...
def get_stats(df: pd.DataFrame,discretization_bins: int = DEFAULT_DISCRETIZATION_BINS) -> pd.DataFrame:
    ...
    def _get_distribution(feature: str):
        ...
        if df[feature].dtype == 'int' or df[feature].dtype == 'float':
            ...
            distribution["keys"], distribution["values"] = [*top_values.to_dict().keys()], [*top_values.to_dict().values()]
            if others_count > 0:
    ...
    features_view = features_view.merge(right=df.describe().T.reset_index(), how="left", left_on="feature", right_on="index")
    features_view = features_view.drop("index", axis=1)
    features_view["count"] = 0
    features_view = features_view.rename(columns={
    ...
    features_view = features_view[["feature", "missing_values", "min", "max", "mean", "std", "1st_percentile",
       "2nd_percentile", "3rd_percentile", "type", "values", "distribution", "output", "sensitive"]]
